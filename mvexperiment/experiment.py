...
from .curve import (
    MODE_DIRECTION_BACKWARD,
    MODE_DIRECTION_FORWARD,
    MODE_DIRECTIONS_PAUSE,
    Segment,
)
...
class DataSet(MvNode):
    ...
    def __init__(self, filename=None, parent=None):
        ...
        self.tip_radius = (
            1000.0  # radius of the tip, used for sphere, in nm, or angle in deg
        )
        self.tip_shape = (
            None  # onest shapes are 'sphere' , 'cone' , 'cylinder', 'pyramid'
        )
        ...
        for line in f:
            ...
            if targeted is True:
                ...
                if target == "Tip radius (um)":
                    self.tip_radius = (
                        toFloat(value) * 1000.0
                    )  # NB: internal units are nm
                elif target == "k (N/m)":
                    self.cantilever_k = toFloat(value)
                elif target == "Calibration factor":
                    self.cantilever_lever = toFloat(
                        value
                    )  # NB: so called geometric factor
    ...
    def createSegments(self):
        ...
        for i in range(len(self.data["time"])):
            if i == len(self.data["time"]) - 1:
                ...
                if (
                    self.data["time"][i] <= nexttime
                    and self.data["time"][i + 1] > nexttime
                ):
                    timefound = True
                if (
    ...
    def header(self):
        ...
        for line in f:
            if line[0 : len(targets[2])] == targets[2]:  # 1.4736e-07m/V
                ...
                self.cantilever_type = line[
                    len(targets[0]) :
                ].strip()  # guess the radius
...
class Jpk(DataSet):
    ...
    def load(self):
        ...
        self.data["z"] = [
            -1.0 * (fd.appr["height (measured)"] * 1e9),
            -1.0 * (fd.retr["height (measured)"] * 1e9),
        ]  # flip z
