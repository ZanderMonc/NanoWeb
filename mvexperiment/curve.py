    ...
    def plot(self, mode="curve", label=None, color="b", alpha=1):
        if mode == "curve":
            ...
            if self.iContact > 0:
                ...
                plt.plot(
                    self.z - offsetX, self.f - offsetY, color=color, ls="-", alpha=alpha
                )
            else:
            ...
            if self.indentation is not None:
                if label is None:
                    ...
                    plt.plot(
                        self.indentation, self.touch, color=color, ls="-", label=label
                    )
                if self.young is not None:
                    if self.youngIThreshold is not None:
                        plt.plot(
                            self.indentation[: self.youngIThreshold],
                            self.hertz(self.indentation[: self.youngIThreshold]),
                            "g-",
                            label="Hertz fit E={}".format(int(self.young * 1e9)),
                        )
                    else:
                        plt.plot(
                            self.indentation,
                            self.hertz(self.indentation),
                            "g-",
                            label="Hertz fit E={}".format(int(self.young * 1e9)),
                        )
            plt.ylabel("Force [nN]")
            ...
            if self.elastography is not None and self.elastography.Ex is not None:
                ...
                if label is None:
                    plt.plot(
                        self.elastography.Ex, self.elastography.Ey, color=color, ls="-"
                    )
                else:
                    plt.plot(
                        self.elastography.Ex,
                        self.elastography.Ey,
                        color=color,
                        ls="-",
                        label=label,
                    )
                if self.elastography.bilData is not None:
                    ...
                    plt.plot(
                        self.elastography.Ex,
                        self.elastography.EBilayer(self.elastography.Ex, ec, eb, t),
                        "g-",
                        label=lbl,
                    )
